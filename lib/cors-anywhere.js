...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    if (req.url.slice(0,7) === "/?code=" && req.method === "POST") { //we'll send our pauth requests here
      ...
      (async () => {
        ...
        let data = {
          ...
          client_secret: process.env[query.client_id+"_SECRET"],
          redirect_uri: process.env[query.client_id+"_URI"],
          code: query.code
        };
        ...
        let auth = await fetch("https://github.com/login/oauth/access_token", {
